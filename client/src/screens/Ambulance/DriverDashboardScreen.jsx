...
const DriverDashboardScreen = ({ navigation }) => {
  ...
  const { isTracking, startBackgroundTracking, stopBackgroundTracking } = useBackgroundLocation();
  ...
  const loadData = async () => {
    try {
      ...
      if (profileResult.success) {
        ...
        setIsOnDuty(profileResult.data.ambulance.status === 'available' || profileResult.data.ambulance.status === 'on_duty');
      }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.driverName}>{profile?.driver?.name || 'Driver'}</Text>
          <Text style={styles.vehicleNumber}>{profile?.vehicleNumber}</Text>
        </View>
        <View style={[styles.statusBadge, { backgroundColor: isOnDuty ? COLORS.success : COLORS.textSecondary }]}>
      ...
      >
          ...
          <View style={styles.dutyHeader}>
            <View>
              ...
              <Text style={styles.dutySubtitle}>
                {isOnDuty ? 'Available for emergencies' : 'Not accepting requests'}
              </Text>
            </View>
            <Switch
              value={isOnDuty}
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity style={styles.actionCard}>
            ...
            <View style={styles.actionContent}>
              ...
              <Text style={styles.actionSubtitle}>Refresh your current position</Text>
            </View>
          ...
          <TouchableOpacity style={styles.actionCard}>
            ...
            <View style={styles.actionContent}>
              ...
              <Text style={styles.actionSubtitle}>Update available equipment</Text>
            </View>
            <Ionicons name="chevron-forward" size={20} color={COLORS.textSecondary} />
          </TouchableOpacity>
        ...
        <Card style={styles.infoCard}>
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoValue}>{profile?.baseHospital?.name || 'Not assigned'}</Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Status:</Text>
            <Text style={[styles.infoValue, { color: isOnDuty ? COLORS.success : COLORS.textSecondary }]}>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingTop: 60, paddingBottom: 20, backgroundColor: COLORS.background },
  ...
  trackingIndicator: { flexDirection: 'row', alignItems: 'center', marginTop: 12, gap: 8 },
